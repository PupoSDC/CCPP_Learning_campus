...
void Mesh::initiateVariables(double cpinput, double cvinput){
    // Assuming T,p and U are defined, updates the remaining flow quantities;
    // And sets the default constants
    ...
    {
        rho[i]   = p[i] / (R * T[i]);
        E[i]     = rho[i] * ( (1/2) * pow(U[i],2)  + p[i] / ( (k-1) * rho[i] ) );      
        macno[i] = sqrt( k * p[i] / rho[i]);
        rhoU[i]  = rho[i] * U[i];
        rhoE[i]  = rho[i] * E[i];
    }   

    updateFluxes(); // Updates fluxes for t=0;
}
...
void Mesh::updateFluxes(){
    // Updates fluxes using Flux vector splitting van Leer

    // Fluxes at the Boundaries ar artificially set to 0
    rhoFlux[0]               = 0; 
    ...
    // Allcoation for the constant part of the flux
    double headerPlusFront;  
    double headerMinusFront; 
    double headerPlusTrail;
    double headerMinusTrail;

    for (int i = 1; i<numberofpoints-1; i++)
    {
        headerPlusFront   = ( 1.0/4.0)*rho[i]  *macno[i]  *pow( (1 + U[i]  /macno[i]  ), 2 ) * 1 / delta;
        headerMinusFront  = (-1.0/4.0)*rho[i+1]*macno[i+1]*pow( (1 - U[i+1]/macno[i+1]), 2 ) * 1 / delta;
        headerPlusTrail   = ( 1.0/4.0)*rho[i-1]*macno[i-1]*pow( (1 + U[i-1]/macno[i-1]), 2 ) * 1 / delta;
        headerMinusTrail  = (-1.0/4.0)*rho[i]  *macno[i]  *pow( (1 - U[i]  /macno[i]  ), 2 ) * 1 / delta;

        rhoFlux[i]   =  ( headerPlusFront  + headerMinusFront ) - (  headerPlusTrail + headerMinusTrail );
  
        rhoUFlux[i]  =  (   headerPlusFront  * 2.0 * macno[i]  /k * ( (k-1.0)/2.0 * U[i]   / macno[i]   + 1.0 ) 
                          + headerMinusFront * 2.0 * macno[i+1]/k * ( (k-1.0)/2.0 * U[i+1] / macno[i+1] - 1.0 ) )
                     -  (   headerPlusTrail  * 2.0 * macno[i-1]/k * ( (k-1.0)/2.0 * U[i-1] / macno[i-1] + 1.0 )
                          + headerMinusTrail * 2.0 * macno[i]  /k * ( (k-1.0)/2.0 * U[i]   / macno[i]   - 1.0 ) );

        rhoEFlux[i]  =  (   headerPlusFront  * 2.0 * pow(macno[i  ],2) / (pow(k,2)-1) * pow( ( (k-1.0)/2.0 * U[i]   / macno[i]   + 1.0 ),2)
                          + headerMinusFront * 2.0 * pow(macno[i+1],2) / (pow(k,2)-1) * pow( ( (k-1.0)/2.0 * U[i+1] / macno[i+1] - 1.0 ),2) )
                     -  (   headerPlusTrail  * 2.0 * pow(macno[i-1],2) / (pow(k,2)-1) * pow( ( (k-1.0)/2.0 * U[i-1] / macno[i-1] + 1.0 ),2)
                          + headerMinusTrail * 2.0 * pow(macno[i  ],2) / (pow(k,2)-1) * pow( ( (k-1.0)/2.0 * U[i]   / macno[i]   - 1.0 ),2) );
       
    }   
}

void Mesh::updateBoundaryConditions(){
    // Temporary (fixed) Boundary conditions
    // Velocity has a dirichlet BC of 0 at the extremeties
    // Energy and density a von Neuman condition
